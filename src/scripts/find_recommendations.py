    ...
    for k in range(n - 1, i, -1):
        ...
        for j in range(k, i, -1):
            ...
            for col_name in list(dfs.values())[k].columns:
                similar_cols = process.extractBests(col_name, list(dfs.values())[j].columns,
                                                    scorer=fuzz.token_sort_ratio, limit=None)
                similar_cols = [col[0] for col in similar_cols if
                                col[1] >= threshold and col[0] != col_name and col[1] < 100]
                ...
                if similar_cols:
                    ...
                    if list(dfs.keys())[k] == list(dfs.keys())[j]:
                        recommendations[f"Recommendation{recommendation_num}"][
                            str(list(dfs.keys())[k]) + "_"] = col_name
                        recommendations[f"Recommendation{recommendation_num}"][list(dfs.keys())[j]] = similar_cols
                        recommendation_num += 1
                    else:
                        recommendations[f"Recommendation{recommendation_num}"][list(dfs.keys())[k]] = col_name
                        recommendations[f"Recommendation{recommendation_num}"][list(dfs.keys())[j]] = similar_cols
                        recommendation_num += 1
...
def sparktfidf(df1, df2, df1_name, df2_name):
    ...
    for columndf1 in df1.columns:
        text1 = ' '.join(df1[columndf1].cast(StringType()))
        for columndf2 in df2.columns:
            text1 = ' '.join(df2[columndf2].cast(StringType()))
            tfidf_matrix = vectorizer.fit_transform([text1] + [text2])
            cosine_similarities = cosine_similarity(tfidf_matrix[0:1], tfidf_matrix[1:])
            if cosine_similarities > 0.8:
                print(
                    f"Similitud de coseno entre {columndf1} de {df1_name} y {columndf2} de {df2_name} tratadas como textos completos: {cosine_similarities}")
    ...
    for k in range(n-1,i,-1):
      ...
      for j in range(k,i,-1):
        ...
        for col_name in list(dfs.values())[k].columns:
            similar_columns = process.extractBests(col_name, list(dfs.values())[j].columns, scorer=fuzz.token_sort_ratio, limit=None)
            similar_cols = [col[0] for col in similar_columns if col[1] >= threshold and col[0] != col_name ]
            
            if similar_cols:
                # print("SIM: ", col_name, similar_cols)
                # similar_cols.append(col_name)
                # column_mapping[df1_name][col_name] = {}
                # column_mapping[df1_name][col_name][df2_name] = similar_cols
                recommendations[f"Recommendation{recommendation_num}"] = {}
                if list(dfs.keys())[k] == list(dfs.keys())[j]: 
                    recommendations[f"Recommendation{recommendation_num}"][(str(list(dfs.keys())[k])+"_").split("/")[-1]] = col_name
                    recommendations[f"Recommendation{recommendation_num}"][str(list(dfs.keys())[j]).split("/")[-1]] = similar_cols
                    recommendation_num += 1
                else:
                    recommendations[f"Recommendation{recommendation_num}"][str(list(dfs.keys())[k]).split("/")[-1]] = col_name
                    recommendations[f"Recommendation{recommendation_num}"][str(list(dfs.keys())[j]).split("/")[-1]] = similar_cols
                    recommendation_num += 1
            ...
            if similar_cols2:
                ...
                if str(list(dfs.keys())[k]) != str(list(dfs.keys())[j]):
                    ...
                    if list(dfs.keys())[k] == list(dfs.keys())[j]: 
                        recommendations[f"Recommendation{recommendation_num}"][(str(list(dfs.keys())[k])+"_").split("/")[-1]] = col_name
                        recommendations[f"Recommendation{recommendation_num}"][str(list(dfs.keys())[j]).split("/")[-1]] = similar_cols2
                        recommendation_num += 1
                    else:
                        recommendations[f"Recommendation{recommendation_num}"][str(list(dfs.keys())[k]).split("/")[-1]] = col_name
                        recommendations[f"Recommendation{recommendation_num}"][str(list(dfs.keys())[j]).split("/")[-1]] = similar_cols2
    ...
    for columndf1 in df1.columns:
        for columndf2 in df2.columns:
            ...
            if similarity[0][0] > 0.8:
                print(f"Similitud de coseno entre {columndf1} de {df1_name} y {columndf2} de {df2_name} tratadas como textos completos: {similarity[0][0]}")
            #print("I")

def sample_dfs():
    for df_key in dfs.keys():
        dfs[df_key] = dfs[df_key].sample(False, 0.1, seed = 274)
    return dfs
    ...
    for columndf1 in df1.columns:
        ...
        for columndf2 in df2.columns:
            ...
            if cosine_similarities > 0.8:
                ...
                print(f"Similitud de coseno entre {columndf1} de {df1_name} y {columndf2} de {df2_name}: {cosine_similarities}")
